...
namespace NinjaTrader.NinjaScript.Indicators.Infinity
{
	...
	public static class FPV3Globals
	{
	...
	public class FootPrintV3 : Indicator
	{
		#region cdArea
			...
			public void setValueArea()
			{
				...
				if(!this.rowItems.IsEmpty)
				{
					...
					while(tmpVol < maxVol)
					{
						...
						upperV = (upperP < maxPrc) ? getVolume(upperP + FPV3Globals.tsv) : -1.0;
						lowerV = (lowerP > minPrc) ? getVolume(lowerP - FPV3Globals.tsv) : -1.0;
						
						if(upperV > lowerV)
						{
							vah	   = upperP + FPV3Globals.tsv;
							tmpVol = tmpVol + upperV; 
							upperP = vah;
						}
						else
						{
							val	   = lowerP - FPV3Globals.tsv;
		...
		protected override void OnStateChange()
		{
			if(State == State.SetDefaults)
			{
				...
				custProfileMap		 = true;
				custProfileMapType	 = FPV3MapDisplayType.Volume;
				...
				footprintHotKey = FPV3Hotkeys.None;
				mapHotKey 		= FPV3Hotkeys.None;
		...
		public void chartControlOnKeyDown(object sender, KeyEventArgs e)
        {
			...
			{
				if(footprintHotKey == FPV3Hotkeys.ShiftSpace)
				{
					toggleFootprint();
				}
				if(custProfileShow && mapHotKey == FPV3Hotkeys.ShiftSpace)
				{
					toggleMap();
				}
			}
			...
			{
				if(footprintHotKey == FPV3Hotkeys.CtrlSpace)
				{
					toggleFootprint();
				}
				if(custProfileShow && mapHotKey == FPV3Hotkeys.CtrlSpace)
				{
					toggleMap();
				}
			}
			else if(custProfileMap)
			{
				if(e.Key == Key.Space)
				{
					if(custProfileMapType == FPV3MapDisplayType.Volume)
					{
						custProfileMapType = FPV3MapDisplayType.Delta;
		...
		protected void InsertWPFControls()
		{
			...
			topMenuItem2SubItem1 = new Gui.Tools.NTMenuItem()
			{
				...
				InputGestureText 	= (mapHotKey == FPV3Hotkeys.ShiftSpace) ? "Shift+Space" : (mapHotKey == FPV3Hotkeys.CtrlSpace) ? "Ctrl+Space" : "",
				Foreground			= Brushes.Silver,
				Margin				= new System.Windows.Thickness(0),
				Padding				= new System.Windows.Thickness(1),
				VerticalAlignment	= VerticalAlignment.Center,
			...
			{
				Header				= (custProfileMapType == FPV3MapDisplayType.Volume) ? "Switch to Delta" : "Switch to Volume",
			...
			topMenuItem3SubItem1 = new Gui.Tools.NTMenuItem()
			{
				...
				InputGestureText 	= (footprintHotKey == FPV3Hotkeys.ShiftSpace) ? "Shift+Space" : (footprintHotKey == FPV3Hotkeys.CtrlSpace) ? "Ctrl+Space" : "",
				Foreground			= Brushes.Silver,
				Margin				= new System.Windows.Thickness(0),
				Padding				= new System.Windows.Thickness(1),
				VerticalAlignment	= VerticalAlignment.Center,
			...
			{
				Header				= (footprintDisplayType == FPV3FootprintDisplayType.Profile) ? "Switch to Numbers" : "Switch to Profile",
				Foreground			= Brushes.Silver,
				Margin				= new System.Windows.Thickness(0),
				Padding				= new System.Windows.Thickness(1),
				VerticalAlignment	= VerticalAlignment.Center,
			...
			topMenuItem3SubItem7 = new Gui.Tools.NTMenuItem()
			{
				...
				Visibility			= (footprintDisplayType == FPV3FootprintDisplayType.Numbers) ? Visibility.Visible : Visibility.Collapsed
			};
		...
		{
			custProfileMapType = (custProfileMapType == FPV3MapDisplayType.Volume) ? FPV3MapDisplayType.Delta : FPV3MapDisplayType.Volume;
			topMenuItem2SubItem2.Header = (custProfileMapType == FPV3MapDisplayType.Volume) ? "Switch to Delta" : "Switch to Volume";
		  	ForceRefresh();
		}
		...
		{
			footprintDisplayType = (footprintDisplayType == FPV3FootprintDisplayType.Profile) ? FPV3FootprintDisplayType.Numbers : FPV3FootprintDisplayType.Profile;
			topMenuItem3SubItem2.Header = (footprintDisplayType == FPV3FootprintDisplayType.Profile) ? "Switch to Numbers" : "Switch to Profile";
		...
		{
			if(footprintDisplayType == FPV3FootprintDisplayType.Profile)
			{
		...
		protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
		{
			...
			try
			{
				...
				bmr += (showFootprint && footprintDisplayType == FPV3FootprintDisplayType.Numbers && footprintDeltaProfile) ? (cellWidth + 2) : 0;
				bmr += (bmr > 0) ? 10 : 0;
		...
		private void drawMap(ChartControl chartControl, ChartScale chartScale)
		{
			try
			{
				...
				double sRng = 0.0;
				float  oRng = (custProfileMapType == FPV3MapDisplayType.Volume) ? mapMaxOpa : mapMaxOpa / 2;
				double size = 0.0;
				float  opac = 0.0f;
				double cont = 0.0;
				...
				for(int i=ChartBars.ToIndex;i>=Math.Max(ChartBars.FromIndex-1, 0);i--)
				{
					if(BarItems.IsValidDataPointAt(i))
					{
						...
						sRng = (custProfileMapType == FPV3MapDisplayType.Volume) ? barItem.custProfile.rowItems[barItem.custProfile.poc].vol : barItem.custProfile.getMaxDta();
						size = 0.0;
						opac = 0.0f;
						cont = 0.0;
						...
						{
							size = (custProfileMapType == FPV3MapDisplayType.Volume) ? ri.Value.vol : Math.Abs(ri.Value.dta);
							...
							opac = (custProfileMapType == FPV3MapDisplayType.Volume && ri.Key == barItem.custProfile.poc) ? opac + 0.3f : opac;
		...
		private void drawTapeStrip(ChartControl chartControl, ChartScale chartScale)
		{
			...
			try
			{
				...
				float x = chartControl.GetXByBarIndex(ChartBars, ChartBars.ToIndex) + cellWidth + 10f;
					  x = (showFootprint && footprintDisplayType == FPV3FootprintDisplayType.Numbers && footprintDeltaProfile) ? x + (cellWidth + 2) : x;
		...
		private void drawFootPrint(ChartControl chartControl, ChartScale chartScale)
		{
			...
			if(footprintRelativeVolume)
			{
				for(int i=ChartBars.FromIndex;i<=ChartBars.ToIndex;i++)
				{
					...
					if(footprintDisplayType == FPV3FootprintDisplayType.Numbers && footprintDeltaProfile)
					{
			...
			for(int i=ChartBars.FromIndex;i<=ChartBars.ToIndex;i++)
			{
				...
				foreach(KeyValuePair<double, RowItem> ri in currBarItem.rowItems)
				{
					...
					barWidth = (footprintDisplayType == FPV3FootprintDisplayType.Numbers) ? cellWidth : (cellWidth / maxVol) * askVol;
					...
					barWidth = (footprintDisplayType == FPV3FootprintDisplayType.Numbers) ? cellWidth : (cellWidth / maxVol) * bidVol;
					barWidth = Math.Round(barWidth);
		...
		private void drawBottomArea(ChartControl chartControl, ChartScale chartScale)
		{
			...
			if(showBottomArea)
			{
				...
				if(bottomAreaType == FPV3BottomAreaType.CumulativeDelta)
				{
				...
				if(bottomAreaType == FPV3BottomAreaType.Delta)
				{
					for(int i=ChartBars.FromIndex;i<=ChartBars.ToIndex;i++)
					{
						if(BarItems.IsValidDataPointAt(i))
				...
				if(bottomAreaType == FPV3BottomAreaType.Volume)
				{
					for(int i=ChartBars.FromIndex;i<=ChartBars.ToIndex;i++)
					{
						if(BarItems.IsValidDataPointAt(i))
		...
		public FPV3MapDisplayType custProfileMapType
		{ get; set; }
		...
		public FPV3FootprintDisplayType footprintDisplayType
		{ get; set; }
		...
		public FPV3BottomAreaType bottomAreaType
		{ get; set; }
		...
		public FPV3Hotkeys footprintHotKey
		{ get; set; }
...
public enum FPV3MapDisplayType
{
	Volume,
	Delta
}

public enum FPV3FootprintDisplayType
{
	Numbers,
	Profile
}

public enum FPV3BottomAreaType
{
	Volume,
	Delta,
	CumulativeDelta
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Infinity.FootPrintV3[] cacheFootPrintV3;
		public Infinity.FootPrintV3 FootPrintV3(int rightMargin, bool showClose, bool paintBars, bool showStackedImbalances, bool prevProfileShow, int prevProfileWidth, bool prevProfileGradient, bool prevProfileValueArea, bool prevProfileExtendVa, bool prevProfileShowDelta, bool prevProfileBar, bool currProfileShow, int currProfileWidth, bool currProfileGradient, bool currProfileValueArea, bool currProfileExtendVa, bool currProfileShowDelta, bool currProfileBar, bool custProfileShow, int custProfileWidth, bool custProfileGradient, bool custProfileValueArea, bool custProfileExtendVa, bool custProfileShowDelta, double custProfilePctValue, int custProfileBarValue, double custProfileVolValue, double custProfileRngValue, bool custProfileMap, FPV3MapDisplayType custProfileMapType, bool showFootprint, FPV3FootprintDisplayType footprintDisplayType, bool footprintImbalances, double minImbalanceRatio, bool footprintDeltaOutline, bool footprintDeltaProfile, bool footprintDeltaGradient, bool footprintGradient, bool footprintRelativeVolume, bool footprintBarVolume, bool footprintBarDelta, bool footprintBarDeltaSwing, bool showBottomArea, FPV3BottomAreaType bottomAreaType, bool bottomAreaGradient, bool bottomAreaLabel, bool bottomTextDelta, bool bottomTextVolume, bool bottomTextCumulativeDelta, bool showTapeStrip, int tapeStripMaxItems, double tapeStripFilter, Brush bullishColor, Brush bearishColor, Brush neutralColor, Brush profileColor, Brush mapColor, Brush pocColor, Brush stackedColor, float profileMaxOpa, float mapMaxOpa, float footprintMaxOpa, float stackedImbOpa, FPV3Hotkeys footprintHotKey, FPV3Hotkeys mapHotKey, bool bullishStackedImbalanceAlert, string bullishStackedImbalanceSound, bool bearishStackedImbalanceAlert, string bearishStackedImbalanceSound, double rBarWidth, float rBarDistance, bool rScaleFixed, double rScaleMax, double rScaleMin, double fBarWidth, float fBarDistance, bool fScaleFixed, double fScaleMax, double fScaleMin)
		{
			return FootPrintV3(Input, rightMargin, showClose, paintBars, showStackedImbalances, prevProfileShow, prevProfileWidth, prevProfileGradient, prevProfileValueArea, prevProfileExtendVa, prevProfileShowDelta, prevProfileBar, currProfileShow, currProfileWidth, currProfileGradient, currProfileValueArea, currProfileExtendVa, currProfileShowDelta, currProfileBar, custProfileShow, custProfileWidth, custProfileGradient, custProfileValueArea, custProfileExtendVa, custProfileShowDelta, custProfilePctValue, custProfileBarValue, custProfileVolValue, custProfileRngValue, custProfileMap, custProfileMapType, showFootprint, footprintDisplayType, footprintImbalances, minImbalanceRatio, footprintDeltaOutline, footprintDeltaProfile, footprintDeltaGradient, footprintGradient, footprintRelativeVolume, footprintBarVolume, footprintBarDelta, footprintBarDeltaSwing, showBottomArea, bottomAreaType, bottomAreaGradient, bottomAreaLabel, bottomTextDelta, bottomTextVolume, bottomTextCumulativeDelta, showTapeStrip, tapeStripMaxItems, tapeStripFilter, bullishColor, bearishColor, neutralColor, profileColor, mapColor, pocColor, stackedColor, profileMaxOpa, mapMaxOpa, footprintMaxOpa, stackedImbOpa, footprintHotKey, mapHotKey, bullishStackedImbalanceAlert, bullishStackedImbalanceSound, bearishStackedImbalanceAlert, bearishStackedImbalanceSound, rBarWidth, rBarDistance, rScaleFixed, rScaleMax, rScaleMin, fBarWidth, fBarDistance, fScaleFixed, fScaleMax, fScaleMin);
		}

		public Infinity.FootPrintV3 FootPrintV3(ISeries<double> input, int rightMargin, bool showClose, bool paintBars, bool showStackedImbalances, bool prevProfileShow, int prevProfileWidth, bool prevProfileGradient, bool prevProfileValueArea, bool prevProfileExtendVa, bool prevProfileShowDelta, bool prevProfileBar, bool currProfileShow, int currProfileWidth, bool currProfileGradient, bool currProfileValueArea, bool currProfileExtendVa, bool currProfileShowDelta, bool currProfileBar, bool custProfileShow, int custProfileWidth, bool custProfileGradient, bool custProfileValueArea, bool custProfileExtendVa, bool custProfileShowDelta, double custProfilePctValue, int custProfileBarValue, double custProfileVolValue, double custProfileRngValue, bool custProfileMap, FPV3MapDisplayType custProfileMapType, bool showFootprint, FPV3FootprintDisplayType footprintDisplayType, bool footprintImbalances, double minImbalanceRatio, bool footprintDeltaOutline, bool footprintDeltaProfile, bool footprintDeltaGradient, bool footprintGradient, bool footprintRelativeVolume, bool footprintBarVolume, bool footprintBarDelta, bool footprintBarDeltaSwing, bool showBottomArea, FPV3BottomAreaType bottomAreaType, bool bottomAreaGradient, bool bottomAreaLabel, bool bottomTextDelta, bool bottomTextVolume, bool bottomTextCumulativeDelta, bool showTapeStrip, int tapeStripMaxItems, double tapeStripFilter, Brush bullishColor, Brush bearishColor, Brush neutralColor, Brush profileColor, Brush mapColor, Brush pocColor, Brush stackedColor, float profileMaxOpa, float mapMaxOpa, float footprintMaxOpa, float stackedImbOpa, FPV3Hotkeys footprintHotKey, FPV3Hotkeys mapHotKey, bool bullishStackedImbalanceAlert, string bullishStackedImbalanceSound, bool bearishStackedImbalanceAlert, string bearishStackedImbalanceSound, double rBarWidth, float rBarDistance, bool rScaleFixed, double rScaleMax, double rScaleMin, double fBarWidth, float fBarDistance, bool fScaleFixed, double fScaleMax, double fScaleMin)
		{
			if (cacheFootPrintV3 != null)
				for (int idx = 0; idx < cacheFootPrintV3.Length; idx++)
					if (cacheFootPrintV3[idx] != null && cacheFootPrintV3[idx].rightMargin == rightMargin && cacheFootPrintV3[idx].showClose == showClose && cacheFootPrintV3[idx].paintBars == paintBars && cacheFootPrintV3[idx].showStackedImbalances == showStackedImbalances && cacheFootPrintV3[idx].prevProfileShow == prevProfileShow && cacheFootPrintV3[idx].prevProfileWidth == prevProfileWidth && cacheFootPrintV3[idx].prevProfileGradient == prevProfileGradient && cacheFootPrintV3[idx].prevProfileValueArea == prevProfileValueArea && cacheFootPrintV3[idx].prevProfileExtendVa == prevProfileExtendVa && cacheFootPrintV3[idx].prevProfileShowDelta == prevProfileShowDelta && cacheFootPrintV3[idx].prevProfileBar == prevProfileBar && cacheFootPrintV3[idx].currProfileShow == currProfileShow && cacheFootPrintV3[idx].currProfileWidth == currProfileWidth && cacheFootPrintV3[idx].currProfileGradient == currProfileGradient && cacheFootPrintV3[idx].currProfileValueArea == currProfileValueArea && cacheFootPrintV3[idx].currProfileExtendVa == currProfileExtendVa && cacheFootPrintV3[idx].currProfileShowDelta == currProfileShowDelta && cacheFootPrintV3[idx].currProfileBar == currProfileBar && cacheFootPrintV3[idx].custProfileShow == custProfileShow && cacheFootPrintV3[idx].custProfileWidth == custProfileWidth && cacheFootPrintV3[idx].custProfileGradient == custProfileGradient && cacheFootPrintV3[idx].custProfileValueArea == custProfileValueArea && cacheFootPrintV3[idx].custProfileExtendVa == custProfileExtendVa && cacheFootPrintV3[idx].custProfileShowDelta == custProfileShowDelta && cacheFootPrintV3[idx].custProfilePctValue == custProfilePctValue && cacheFootPrintV3[idx].custProfileBarValue == custProfileBarValue && cacheFootPrintV3[idx].custProfileVolValue == custProfileVolValue && cacheFootPrintV3[idx].custProfileRngValue == custProfileRngValue && cacheFootPrintV3[idx].custProfileMap == custProfileMap && cacheFootPrintV3[idx].custProfileMapType == custProfileMapType && cacheFootPrintV3[idx].showFootprint == showFootprint && cacheFootPrintV3[idx].footprintDisplayType == footprintDisplayType && cacheFootPrintV3[idx].footprintImbalances == footprintImbalances && cacheFootPrintV3[idx].minImbalanceRatio == minImbalanceRatio && cacheFootPrintV3[idx].footprintDeltaOutline == footprintDeltaOutline && cacheFootPrintV3[idx].footprintDeltaProfile == footprintDeltaProfile && cacheFootPrintV3[idx].footprintDeltaGradient == footprintDeltaGradient && cacheFootPrintV3[idx].footprintGradient == footprintGradient && cacheFootPrintV3[idx].footprintRelativeVolume == footprintRelativeVolume && cacheFootPrintV3[idx].footprintBarVolume == footprintBarVolume && cacheFootPrintV3[idx].footprintBarDelta == footprintBarDelta && cacheFootPrintV3[idx].footprintBarDeltaSwing == footprintBarDeltaSwing && cacheFootPrintV3[idx].showBottomArea == showBottomArea && cacheFootPrintV3[idx].bottomAreaType == bottomAreaType && cacheFootPrintV3[idx].bottomAreaGradient == bottomAreaGradient && cacheFootPrintV3[idx].bottomAreaLabel == bottomAreaLabel && cacheFootPrintV3[idx].bottomTextDelta == bottomTextDelta && cacheFootPrintV3[idx].bottomTextVolume == bottomTextVolume && cacheFootPrintV3[idx].bottomTextCumulativeDelta == bottomTextCumulativeDelta && cacheFootPrintV3[idx].showTapeStrip == showTapeStrip && cacheFootPrintV3[idx].tapeStripMaxItems == tapeStripMaxItems && cacheFootPrintV3[idx].tapeStripFilter == tapeStripFilter && cacheFootPrintV3[idx].bullishColor == bullishColor && cacheFootPrintV3[idx].bearishColor == bearishColor && cacheFootPrintV3[idx].neutralColor == neutralColor && cacheFootPrintV3[idx].profileColor == profileColor && cacheFootPrintV3[idx].mapColor == mapColor && cacheFootPrintV3[idx].pocColor == pocColor && cacheFootPrintV3[idx].stackedColor == stackedColor && cacheFootPrintV3[idx].profileMaxOpa == profileMaxOpa && cacheFootPrintV3[idx].mapMaxOpa == mapMaxOpa && cacheFootPrintV3[idx].footprintMaxOpa == footprintMaxOpa && cacheFootPrintV3[idx].stackedImbOpa == stackedImbOpa && cacheFootPrintV3[idx].footprintHotKey == footprintHotKey && cacheFootPrintV3[idx].mapHotKey == mapHotKey && cacheFootPrintV3[idx].bullishStackedImbalanceAlert == bullishStackedImbalanceAlert && cacheFootPrintV3[idx].bullishStackedImbalanceSound == bullishStackedImbalanceSound && cacheFootPrintV3[idx].bearishStackedImbalanceAlert == bearishStackedImbalanceAlert && cacheFootPrintV3[idx].bearishStackedImbalanceSound == bearishStackedImbalanceSound && cacheFootPrintV3[idx].rBarWidth == rBarWidth && cacheFootPrintV3[idx].rBarDistance == rBarDistance && cacheFootPrintV3[idx].rScaleFixed == rScaleFixed && cacheFootPrintV3[idx].rScaleMax == rScaleMax && cacheFootPrintV3[idx].rScaleMin == rScaleMin && cacheFootPrintV3[idx].fBarWidth == fBarWidth && cacheFootPrintV3[idx].fBarDistance == fBarDistance && cacheFootPrintV3[idx].fScaleFixed == fScaleFixed && cacheFootPrintV3[idx].fScaleMax == fScaleMax && cacheFootPrintV3[idx].fScaleMin == fScaleMin && cacheFootPrintV3[idx].EqualsInput(input))
						return cacheFootPrintV3[idx];
			return CacheIndicator<Infinity.FootPrintV3>(new Infinity.FootPrintV3(){ rightMargin = rightMargin, showClose = showClose, paintBars = paintBars, showStackedImbalances = showStackedImbalances, prevProfileShow = prevProfileShow, prevProfileWidth = prevProfileWidth, prevProfileGradient = prevProfileGradient, prevProfileValueArea = prevProfileValueArea, prevProfileExtendVa = prevProfileExtendVa, prevProfileShowDelta = prevProfileShowDelta, prevProfileBar = prevProfileBar, currProfileShow = currProfileShow, currProfileWidth = currProfileWidth, currProfileGradient = currProfileGradient, currProfileValueArea = currProfileValueArea, currProfileExtendVa = currProfileExtendVa, currProfileShowDelta = currProfileShowDelta, currProfileBar = currProfileBar, custProfileShow = custProfileShow, custProfileWidth = custProfileWidth, custProfileGradient = custProfileGradient, custProfileValueArea = custProfileValueArea, custProfileExtendVa = custProfileExtendVa, custProfileShowDelta = custProfileShowDelta, custProfilePctValue = custProfilePctValue, custProfileBarValue = custProfileBarValue, custProfileVolValue = custProfileVolValue, custProfileRngValue = custProfileRngValue, custProfileMap = custProfileMap, custProfileMapType = custProfileMapType, showFootprint = showFootprint, footprintDisplayType = footprintDisplayType, footprintImbalances = footprintImbalances, minImbalanceRatio = minImbalanceRatio, footprintDeltaOutline = footprintDeltaOutline, footprintDeltaProfile = footprintDeltaProfile, footprintDeltaGradient = footprintDeltaGradient, footprintGradient = footprintGradient, footprintRelativeVolume = footprintRelativeVolume, footprintBarVolume = footprintBarVolume, footprintBarDelta = footprintBarDelta, footprintBarDeltaSwing = footprintBarDeltaSwing, showBottomArea = showBottomArea, bottomAreaType = bottomAreaType, bottomAreaGradient = bottomAreaGradient, bottomAreaLabel = bottomAreaLabel, bottomTextDelta = bottomTextDelta, bottomTextVolume = bottomTextVolume, bottomTextCumulativeDelta = bottomTextCumulativeDelta, showTapeStrip = showTapeStrip, tapeStripMaxItems = tapeStripMaxItems, tapeStripFilter = tapeStripFilter, bullishColor = bullishColor, bearishColor = bearishColor, neutralColor = neutralColor, profileColor = profileColor, mapColor = mapColor, pocColor = pocColor, stackedColor = stackedColor, profileMaxOpa = profileMaxOpa, mapMaxOpa = mapMaxOpa, footprintMaxOpa = footprintMaxOpa, stackedImbOpa = stackedImbOpa, footprintHotKey = footprintHotKey, mapHotKey = mapHotKey, bullishStackedImbalanceAlert = bullishStackedImbalanceAlert, bullishStackedImbalanceSound = bullishStackedImbalanceSound, bearishStackedImbalanceAlert = bearishStackedImbalanceAlert, bearishStackedImbalanceSound = bearishStackedImbalanceSound, rBarWidth = rBarWidth, rBarDistance = rBarDistance, rScaleFixed = rScaleFixed, rScaleMax = rScaleMax, rScaleMin = rScaleMin, fBarWidth = fBarWidth, fBarDistance = fBarDistance, fScaleFixed = fScaleFixed, fScaleMax = fScaleMax, fScaleMin = fScaleMin }, input, ref cacheFootPrintV3);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Infinity.FootPrintV3 FootPrintV3(int rightMargin, bool showClose, bool paintBars, bool showStackedImbalances, bool prevProfileShow, int prevProfileWidth, bool prevProfileGradient, bool prevProfileValueArea, bool prevProfileExtendVa, bool prevProfileShowDelta, bool prevProfileBar, bool currProfileShow, int currProfileWidth, bool currProfileGradient, bool currProfileValueArea, bool currProfileExtendVa, bool currProfileShowDelta, bool currProfileBar, bool custProfileShow, int custProfileWidth, bool custProfileGradient, bool custProfileValueArea, bool custProfileExtendVa, bool custProfileShowDelta, double custProfilePctValue, int custProfileBarValue, double custProfileVolValue, double custProfileRngValue, bool custProfileMap, FPV3MapDisplayType custProfileMapType, bool showFootprint, FPV3FootprintDisplayType footprintDisplayType, bool footprintImbalances, double minImbalanceRatio, bool footprintDeltaOutline, bool footprintDeltaProfile, bool footprintDeltaGradient, bool footprintGradient, bool footprintRelativeVolume, bool footprintBarVolume, bool footprintBarDelta, bool footprintBarDeltaSwing, bool showBottomArea, FPV3BottomAreaType bottomAreaType, bool bottomAreaGradient, bool bottomAreaLabel, bool bottomTextDelta, bool bottomTextVolume, bool bottomTextCumulativeDelta, bool showTapeStrip, int tapeStripMaxItems, double tapeStripFilter, Brush bullishColor, Brush bearishColor, Brush neutralColor, Brush profileColor, Brush mapColor, Brush pocColor, Brush stackedColor, float profileMaxOpa, float mapMaxOpa, float footprintMaxOpa, float stackedImbOpa, FPV3Hotkeys footprintHotKey, FPV3Hotkeys mapHotKey, bool bullishStackedImbalanceAlert, string bullishStackedImbalanceSound, bool bearishStackedImbalanceAlert, string bearishStackedImbalanceSound, double rBarWidth, float rBarDistance, bool rScaleFixed, double rScaleMax, double rScaleMin, double fBarWidth, float fBarDistance, bool fScaleFixed, double fScaleMax, double fScaleMin)
		{
			return indicator.FootPrintV3(Input, rightMargin, showClose, paintBars, showStackedImbalances, prevProfileShow, prevProfileWidth, prevProfileGradient, prevProfileValueArea, prevProfileExtendVa, prevProfileShowDelta, prevProfileBar, currProfileShow, currProfileWidth, currProfileGradient, currProfileValueArea, currProfileExtendVa, currProfileShowDelta, currProfileBar, custProfileShow, custProfileWidth, custProfileGradient, custProfileValueArea, custProfileExtendVa, custProfileShowDelta, custProfilePctValue, custProfileBarValue, custProfileVolValue, custProfileRngValue, custProfileMap, custProfileMapType, showFootprint, footprintDisplayType, footprintImbalances, minImbalanceRatio, footprintDeltaOutline, footprintDeltaProfile, footprintDeltaGradient, footprintGradient, footprintRelativeVolume, footprintBarVolume, footprintBarDelta, footprintBarDeltaSwing, showBottomArea, bottomAreaType, bottomAreaGradient, bottomAreaLabel, bottomTextDelta, bottomTextVolume, bottomTextCumulativeDelta, showTapeStrip, tapeStripMaxItems, tapeStripFilter, bullishColor, bearishColor, neutralColor, profileColor, mapColor, pocColor, stackedColor, profileMaxOpa, mapMaxOpa, footprintMaxOpa, stackedImbOpa, footprintHotKey, mapHotKey, bullishStackedImbalanceAlert, bullishStackedImbalanceSound, bearishStackedImbalanceAlert, bearishStackedImbalanceSound, rBarWidth, rBarDistance, rScaleFixed, rScaleMax, rScaleMin, fBarWidth, fBarDistance, fScaleFixed, fScaleMax, fScaleMin);
		}

		public Indicators.Infinity.FootPrintV3 FootPrintV3(ISeries<double> input , int rightMargin, bool showClose, bool paintBars, bool showStackedImbalances, bool prevProfileShow, int prevProfileWidth, bool prevProfileGradient, bool prevProfileValueArea, bool prevProfileExtendVa, bool prevProfileShowDelta, bool prevProfileBar, bool currProfileShow, int currProfileWidth, bool currProfileGradient, bool currProfileValueArea, bool currProfileExtendVa, bool currProfileShowDelta, bool currProfileBar, bool custProfileShow, int custProfileWidth, bool custProfileGradient, bool custProfileValueArea, bool custProfileExtendVa, bool custProfileShowDelta, double custProfilePctValue, int custProfileBarValue, double custProfileVolValue, double custProfileRngValue, bool custProfileMap, FPV3MapDisplayType custProfileMapType, bool showFootprint, FPV3FootprintDisplayType footprintDisplayType, bool footprintImbalances, double minImbalanceRatio, bool footprintDeltaOutline, bool footprintDeltaProfile, bool footprintDeltaGradient, bool footprintGradient, bool footprintRelativeVolume, bool footprintBarVolume, bool footprintBarDelta, bool footprintBarDeltaSwing, bool showBottomArea, FPV3BottomAreaType bottomAreaType, bool bottomAreaGradient, bool bottomAreaLabel, bool bottomTextDelta, bool bottomTextVolume, bool bottomTextCumulativeDelta, bool showTapeStrip, int tapeStripMaxItems, double tapeStripFilter, Brush bullishColor, Brush bearishColor, Brush neutralColor, Brush profileColor, Brush mapColor, Brush pocColor, Brush stackedColor, float profileMaxOpa, float mapMaxOpa, float footprintMaxOpa, float stackedImbOpa, FPV3Hotkeys footprintHotKey, FPV3Hotkeys mapHotKey, bool bullishStackedImbalanceAlert, string bullishStackedImbalanceSound, bool bearishStackedImbalanceAlert, string bearishStackedImbalanceSound, double rBarWidth, float rBarDistance, bool rScaleFixed, double rScaleMax, double rScaleMin, double fBarWidth, float fBarDistance, bool fScaleFixed, double fScaleMax, double fScaleMin)
		{
			return indicator.FootPrintV3(input, rightMargin, showClose, paintBars, showStackedImbalances, prevProfileShow, prevProfileWidth, prevProfileGradient, prevProfileValueArea, prevProfileExtendVa, prevProfileShowDelta, prevProfileBar, currProfileShow, currProfileWidth, currProfileGradient, currProfileValueArea, currProfileExtendVa, currProfileShowDelta, currProfileBar, custProfileShow, custProfileWidth, custProfileGradient, custProfileValueArea, custProfileExtendVa, custProfileShowDelta, custProfilePctValue, custProfileBarValue, custProfileVolValue, custProfileRngValue, custProfileMap, custProfileMapType, showFootprint, footprintDisplayType, footprintImbalances, minImbalanceRatio, footprintDeltaOutline, footprintDeltaProfile, footprintDeltaGradient, footprintGradient, footprintRelativeVolume, footprintBarVolume, footprintBarDelta, footprintBarDeltaSwing, showBottomArea, bottomAreaType, bottomAreaGradient, bottomAreaLabel, bottomTextDelta, bottomTextVolume, bottomTextCumulativeDelta, showTapeStrip, tapeStripMaxItems, tapeStripFilter, bullishColor, bearishColor, neutralColor, profileColor, mapColor, pocColor, stackedColor, profileMaxOpa, mapMaxOpa, footprintMaxOpa, stackedImbOpa, footprintHotKey, mapHotKey, bullishStackedImbalanceAlert, bullishStackedImbalanceSound, bearishStackedImbalanceAlert, bearishStackedImbalanceSound, rBarWidth, rBarDistance, rScaleFixed, rScaleMax, rScaleMin, fBarWidth, fBarDistance, fScaleFixed, fScaleMax, fScaleMin);
		}
	}
